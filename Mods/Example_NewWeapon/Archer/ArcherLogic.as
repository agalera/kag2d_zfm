...
void onInit( CBlob@ this )
{
	...
	AddIconToken( "$M16$", "M16.png", Vec2f(16,32), 7 );
	// CBlob@ mat = server_CreateBlob("mat_m16");
...
void ManageBow( CBlob@ this, ArcherInfo@ archer, RunnerMoveVars@ moveVars )
{
	...
	if (charge_state != ArcherParams::stabbing )
	{
		...
		{
			hasarrow = SetBackDefaultArrows( this, archer );
		}
...
void ManageGun( CBlob@ this, ArcherInfo@ archer, RunnerMoveVars@ moveVars )
{
	...
	if (this.isKeyPressed(key_action1) && archer.charge_time == 0)
	{
		if (ismyplayer){
			ShootBullet( this, pos+ Vec2f(0.0f, this.isFacingLeft() ? -4.0f : -1.0f), this.getAimPos() + Vec2f(0.0f,-3.0f), ArcherParams::m16_bullet_speed );
		}
		archer.charge_time = ArcherParams::m16_fire_interval;
	}
	...
	// set back to default ammo

	if ((getGameTime()+this.getNetworkID())%10 == 0)
	{
		archer.has_arrow = SetBackDefaultArrows( this, archer );
	}	
}

bool SetBackDefaultArrows( CBlob@ this, ArcherInfo@ archer )
{
	bool has = hasArrows( this );
...
void ShootBullet( CBlob @this, Vec2f pos, Vec2f aimpos, f32 speed )
{
    if (canSend(this))
	{ // player or bot
		Vec2f arrowVel = (aimpos- pos).RotateBy(0.0f,Vec2f(0,0));
		arrowVel.Normalize();
		arrowVel *= speed;
		CBitStream params;
...
void onCommand( CBlob@ this, u8 cmd, CBitStream @params )
{
    ...
    else if (cmd == this.getCommandID("shoot bullet"))
    {
        ...
        if (getNet().isServer())
        {
            ...
            {
				// fire arrow?
				bullet.set_u8("arrow type", arrowType );
				bullet.Init();
