...
void LoadSprites( CSprite@ this )
{
    ...
    CSpriteLayer@ m16_layer = this.addSpriteLayer( "m16", "M16.png" , 32, 16 );

    if (m16_layer !is null)
    {
        Animation@ animdefault = m16_layer.addAnimation( "m16 default", 0, false );
        animdefault.AddFrame(0);
        animdefault.AddFrame(1);
        animdefault.AddFrame(2);
        animdefault.AddFrame(3);

        m16_layer.SetOffset( Vec2f(-1.0f,5.0f+config_offset) );
        m16_layer.SetAnimation("m16 default");
        m16_layer.SetVisible(false);
    }    
}
...
void onTick( CSprite@ this )
{
    ...
    const bool firing = IsFiring(blob) || legolas || (archer.arrow_type == ArrowType::m16 && !down && archer.has_arrow);
    ...
    if (firing)
    {
        ...
        if (archer.arrow_type == ArrowType::m16) {
            DrawGun( this, blob, archer, armangle, arrowType, armOffset );        
        }
        else{
    ...
    else
    {
        ...
        this.getSpriteLayer( "m16" ).SetVisible(false);
    }
...
void DrawGun( CSprite@ this, CBlob@ blob, ArcherInfo@ archer, f32 armangle, const u8 arrowType, Vec2f armOffset )
{
    f32 sign = (this.isFacingLeft()?1.0f:-1.0f);
    CSpriteLayer@ m16_layer = this.getSpriteLayer( "m16" );
    const bool inair = (!blob.isOnGround() && !blob.isOnLadder());
    m16_layer.animation.frame = blob.isKeyPressed(key_action1) ? (archer.charge_time == 0 ? 1+XORRandom(3) : 0) : 0;
    int walk_offset = this.animation.frame%2 + (inair ? 2 : 0); // bip-bop
    setArmValues(m16_layer,true,armangle,0.5f,"m16 default",Vec2f(-8.0f*sign,2.0f), armOffset 
        + Vec2f(-6 - int(m16_layer.animation.frame%2),-2 + walk_offset ) ); 
