...
bool onServerProcessChat(CRules@ this, const string& in text_in, string& out text_out, CPlayer@ player)
{
	...
	{
		CPlayer@ bot = AddBot("JuanitaMayonesa");
		return true;
	}
	...
	if (admin || superadmin || sv_test)
	{
		...
		if (text_in == "!tree" && player.isMod())
		{
			server_MakeSeed(pos, "tree_pine", 600, 1, 16);
		}
		else if (text_in == "!btree" && player.isMod())
		{
			server_MakeSeed(pos, "tree_bushy", 400, 2, 16);
		}
		else if (text_in == "!stones" && player.isMod())
		{
		...
		else if (text_in == "!whitepage" && player.isMod())
		{
			CBlob@ b = server_CreateBlob("whitepage", team, pos);

			if (b !is null)
			{
				b.server_SetQuantity(9);
			}
		}		
		else if (text_in == "!arrows" && player.isMod())
		{
			for (int i = 0; i < 3; i++)
			{
		...
		else if (text_in == "!bombs" && player.isMod())
		{
			//  for (int i = 0; i < 3; i++)
		...
		else if (text_in.substr(0, 1) == "!" && player.isMod())
		{
			...
			if (tokens.length > 1)
			{
				if (tokens[0] == "!crate" && player.isMod())
				{
					int frame = tokens[1] == "catapult" ? 1 : 0;
					string description = tokens.length > 2 ? tokens[2] : tokens[1];
					server_MakeCrate(tokens[1], description, frame, -1, Vec2f(pos.x, pos.y));
				}
				else if (tokens[0] == "!team" && player.isMod())
				{
					int team = parseInt(tokens[1]);
					blob.server_setTeamNum(team);
				}
				else if (tokens[0] == "!spawn" && player.isMod())
				{
					string s = tokens[1];
				...
				else if (tokens[0] == "!zombie" && player.isMod())
				{
