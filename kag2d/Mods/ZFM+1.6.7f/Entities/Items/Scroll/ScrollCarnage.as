		...
		if (caller !is null)
		{
			...
			if (this.getMap().getBlobsInRadius( this.getPosition(), 500.0f, @blobsInRadius )) 
			{
				for (uint i = 0; i < blobsInRadius.length; i++)
				{
					...
					if (b.getTeamNum() != team && b.hasTag("flesh") && !b.hasTag("undeadplayer") && !b.hasTag("survivorplayer"))
					{
						ParticleZombieLightning( b.getPosition() ); 
						if (getNet().isServer())
							caller.server_Hit( b, this.getPosition(), Vec2f(0,0), 20.0f, Hitters::suddengib, true );
						hit = true;
					}
				}
			}
