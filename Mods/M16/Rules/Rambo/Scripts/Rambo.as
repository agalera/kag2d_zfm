...
//Rambo gamemode logic script

#define SERVER_ONLY

#include "Rambo_Structs.as";
...
void Config(RamboCore@ this)
{
	CRules@ rules = getRules();

	//load cfg
	if (rules.exists("rambo_costs_config"))
	   cost_config_file = rules.get_string("rambo_costs_config");
	...
	// modifies if the fall damage velocity is higher or lower - Rambo has lower velocity
	rules.set_f32("fall vel modifier", cfg.read_f32("fall_dmg_nerf", 1.3f));
}

//Rambo spawn system

shared class RamboSpawns : RespawnSystem
{
	RamboCore@ Rambo_core;
	...
	void SetCore(RulesCore@ _core)
	{
		...
		@Rambo_core = cast<RamboCore@>(core);
	}

	void Update()
	{
		for (uint team_num = 0; team_num < Rambo_core.teams.length; ++team_num )
		{
			RamboTeamInfo@ team = cast<RamboTeamInfo@>( Rambo_core.teams[team_num] );

			for (uint i = 0; i < team.spawns.length; i++)
			{
				RamboPlayerInfo@ info = cast<RamboPlayerInfo@>(team.spawns[i]);
	...
	void UpdateSpawnTime(RamboPlayerInfo@ info, int i)
	{
		//default
		...
		bool no_respawn = Rambo_core.rules.isMatchRunning() ? huge_respawn : false;
		if(no_respawn)
		{
			spawn_property = 253;
		}
		...
		string propname = "rambo spawn time "+info.username;
		Rambo_core.rules.set_u8( propname, spawn_property );
		if (info !is null && info.can_spawn_time >= 0)
		{
			Rambo_core.rules.SyncToPlayer( propname, getPlayerByUsername(info.username) );
		}
	}
	...
	{
		RamboPlayerInfo@ info = cast<RamboPlayerInfo@>(p_info);

		if (info is null) { warn("Rambo LOGIC: Couldn't get player info ( in bool canSpawnPlayer(PlayerInfo@ p_info) ) "); return false; }
	...
	void RemovePlayerFromSpawn(PlayerInfo@ p_info)
	{
		...
		if (info is null) { warn("Rambo LOGIC: Couldn't get player info ( in void RemovePlayerFromSpawn(PlayerInfo@ p_info) )"); return; }

		string propname = "rambo spawn time "+info.username;
		
		for (uint i = 0; i < Rambo_core.teams.length; i++)
		{
			RamboTeamInfo@ team = cast<RamboTeamInfo@>(Rambo_core.teams[i]);
		...
		Rambo_core.rules.set_u8( propname, 255 ); //not respawning
		Rambo_core.rules.SyncToPlayer( propname, getPlayerByUsername(info.username) ); 
	...
	void AddPlayerToSpawn( CPlayer@ player )
	{
		...
		u32 tickspawndelay = u32(Rambo_core.spawnTime);

		print("ADD SPAWN FOR " + player.getUsername());
		RamboPlayerInfo@ info = cast<RamboPlayerInfo@>(core.getInfoFromPlayer(player));

		if (info is null) { warn("Rambo LOGIC: Couldn't get player info  ( in void AddPlayerToSpawn(CPlayer@ player) )"); return; }

		info.team = 0; // force team 0
		if (info.team < Rambo_core.teams.length)
		{
			RamboTeamInfo@ team = cast<RamboTeamInfo@>(Rambo_core.teams[info.team]);
		...
		{
			error("PLAYER TEAM NOT SET CORRECTLY! " + info.team + "/" + Rambo_core.teams.length);
		}
	}
...
shared class RamboCore : RulesCore
{
	s32 warmUpTime;
	s32 gameDuration;
	s32 spawnTime;
	...
	RamboSpawns@ rambo_spawns;

	RamboCore() {}

	RamboCore(CRules@ _rules, RespawnSystem@ _respawns )
	{
	...
	void Setup(CRules@ _rules = null, RespawnSystem@ _respawns = null)
	{
		...
		@rambo_spawns = cast<RamboSpawns@>(_respawns);
	 //   server_CreateBlob( "Entities/Meta/RamboMusic.cfg" );
	...
	{
		CBlob@[] m16s;
		getBlobsByName( "m16", @m16s );

		if (m16s.length == 0)
		{
			CBlob@ m16 = server_CreateBlob( "m16");
			if (m16 !is null)
			{
				CMap@ map = getMap();				
				m16.setPosition( Vec2f( map.tilemapwidth*0.5f, map.tilemapheight*0.2f ) * map.tilesize );
			}
		}	
	}
	...
	{			
		...
		else if (ticksToStart > 0 && rules.isWarmup()) //is the start of the game, spawn everyone + give mats
		{
			...
			if (players_in_small_team == -1 || (getGameTime() % 30) == 4)
			{
				...
				{
					RamboTeamInfo@ team = cast<RamboTeamInfo@>( teams[team_num] );
	...
	void updateHUD()
	{
		...
		for (uint team_num = 0; team_num < teams.length; ++team_num )
		{
			...
			{
				RamboPlayerInfo@ player = cast<RamboPlayerInfo@>( players[player_num] );
			...
			if (team.spawns.length > 0 && !rules.isIntermission()) {
				u32 st = cast<RamboPlayerInfo@>(team.spawns[0]).can_spawn_time;
				if(st < 200)
				{
		...
		rules.set_CBitStream("rambo_serialised_team_hud",serialised_team_hud);
		rules.Sync("rambo_serialised_team_hud",true);
	}
	...
	{
		return getPlayerCount() > minimum_players_in_team;
	}
	...
	{
		RamboTeamInfo t(teams.length, team.getName());
		teams.push_back(t);
	}

	void AddPlayer(CPlayer@ player, u8 team = 0, string default_config = "")
	{
		RamboPlayerInfo p(player.getUsername(), player.getTeamNum(), player.isBot() ? "knight" : (XORRandom(512) >= 256 ? "knight" : "archer") );
		players.push_back(p);
	...
	void SetupBases()
	{
		...
		if (map !is null)
		{
			...
			{
				warn("Rambo: Blue spawn marker not found on map");
			...
			{
				warn("Rambo: Red spawn marker not found on map");
		...
		if(sudden_death)
		{
			...
			if(teams_alive_count == 1)
			{
				for(int i = 0; i < teams.length; i++)
				{
					if(teams_alive[i])
					{
						@winteam = cast<RamboTeamInfo@>( teams[i] );
						winteamIndex = i;
						team_wins_on_end = i;
					}
				}
	...
	void addKill(int team)
	{
		...
		{
			RamboTeamInfo@ team_info = cast<RamboTeamInfo@>( teams[team] );
			team_info.kills++;
		}
	}
