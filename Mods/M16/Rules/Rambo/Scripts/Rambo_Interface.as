...
void onRender( CRules@ this )
{
    ...
    if (serialised_team_hud.getBytesUsed() > 10)
    {
        ...
        if (serialised_team_hud.saferead_u16(check) && check == 0x5afe)
        {
            ...
            {
                Rambo_HUD hud(serialised_team_hud);
                ...
                {
                    kills = "KILLS: "+hud.kills+"/"+hud.kills_limit;
                }
    ...
    for (int i = 0; i < m16s.length; i++)
    {
        CBlob@ m16 = m16s[i];
        if (!m16.isAttached())
        {
            Vec2f pos = getDriver().getScreenPosFromWorldPos( m16.getPosition() );
            Vec2f bounce;
            bounce.y = Maths::Sin( getGameTime()*0.33 ) * 15.0f;
            GUI::DrawArrow2D( pos - Vec2f(0,80) + bounce, pos- Vec2f(0,20) + bounce, SColor(255, 255, 255, 0) );
        }
    }    
}
