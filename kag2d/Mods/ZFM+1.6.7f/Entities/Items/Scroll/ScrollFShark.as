...
void GetButtonsFor( CBlob@ this, CBlob@ caller )
{
	...
	if (kek == 0)
	{	
		caller.CreateGenericButton( 11, Vec2f_zero, this, this.getCommandID("summon sharks"), "Use this to summon a Friendly Shark at your mouse position.", params );
	}
}
...
void onCommand( CBlob@ this, u8 cmd, CBitStream @params )
{
	...
	if (cmd == this.getCommandID("summon sharks"))
	{
		...
		CBlob@ shark;
		CBlob@[] sharksCount;
		getBlobsByName("fshark", @sharksCount);
		
		for (int i = 0; i < sharksToSpawnCount; i++)
		{
			if (sharksCount.length >= maxSharks)
			{
				@shark = server_CreateBlob( "fshark", caller.getTeamNum(), this.getPosition()); 
			...
			{
				@shark = server_CreateBlob( "fshark", caller.getTeamNum(), caller.getAimPos()); 
			}
