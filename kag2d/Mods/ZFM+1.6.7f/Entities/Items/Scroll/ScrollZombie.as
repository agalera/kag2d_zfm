...
void onCommand( CBlob@ this, u8 cmd, CBitStream @params )
{
	...
	if (cmd == this.getCommandID("summon zombies"))
	{
		...
		CBlob@ zombie;
		CBlob@[] zombiesCount;
		getBlobsByName("zombie2", @zombiesCount);
		
		for (int i = 0; i < zombiesToSpawnCount; i++)
		{
			if (zombiesCount.length >= maxZombies)
			{
				@zombie = server_CreateBlob( "zombie2", caller.getTeamNum(), this.getPosition()); 
			...
			{
				@zombie = server_CreateBlob( "zombie2", caller.getTeamNum(), caller.getAimPos()); 
			}
